...
public class MvCtlgCntrlrUsingRestTemplate_01_LearnMore {
	private static final Logger log = LoggerFactory.getLogger(MvCtlgCntrlrUsingRestTemplate_01_LearnMore.class);
	@Autowired
	...
	private ClientResponse getRatingsAsClientResponse(@Valid @PathVariable("userId") final String USER_ID) { // We get 'ResponseEntity' from other service
		...
		List<CatalogItemDto> catalogs = ratings.stream().map(r -> {
